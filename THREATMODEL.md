...
- **SOCKS circuit isolation (Rust TUI outbound, honest):** Messenger `socks5_send` / `socks5_connect` negotiate RFC1929 username/password with tags derived from the destination v3 onion (`socks_isolation_credentials`, domain-separated SHA-256). With Tor’s default `IsolateSOCKSAuth`, distinct peers get distinct circuits. Tags are not shared secrets, are never printed by `:evidence`/status, and do not authenticate the client to Tor. Inbound HS accept remains a single local listener (isolation applies to outbound CONNECT only). Not a substitute for Tor bridges or for defending a compromised Tor client.
