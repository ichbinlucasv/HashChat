...
- **ContactAddress / profile sharing**: Implemented hashchat://contact/v1/<onion>/<len:hexpub> links + parse/generate roundtrip in Haskell (Contact.hs). Wired into both thin CLI and real Brick TUI (app-desktop/TUI.hs) with :my-contact / :add-contact. ConnectionRequest mirror for the "scanner replies" flow.
  ...
  - Extreme mode (Rust TUI): `:my-contact` / signed contact-link **display/export** is refused via `NetConfig::extreme_blocks_contact_export()` (minimize link sharing). Local `:sas` fingerprint comparison remains allowed (short status; no signed URI echo). Android contact-QR generation gating remains a separate follow-up.
...
- **Extreme / posture expansion**: Android still gates voice/groups/export under Extreme/strict. Rust TUI Extreme is first-class for metadata surfaces that existed soft: Tor-only mode lock, refuse `:my-contact` export, refuse group/voice stubs, short SAS / onion-tail status under Extreme; `:help` / `:status` announce locks. Not a claim of full Extreme feature parity with the design doc (no compile-time strip, session persistence still exists).
