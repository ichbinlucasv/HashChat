...
echo "  HASHCHAT_DEMO=filter   ./run-tui   # Normal user: :filter or / to narrow contacts list (new rec)"
echo "  HASHCHAT_DEMO=voice-sending ./run-tui # 'v' + visible sendingVoiceTo per contact + banner (rec1 polish)"
...
echo "Run this on your real Fedora (with Tor/i2pd if possible). This covers ALL states from the priority table + NEW normal-user recs (filter/search, voice sending indicator + per-contact feedback, :status panel, QR tips, latest Phase3). Tails/Qubes: adapt grim to their screenshot tools + use qvm-copy for evidence."
echo ""
...
echo "  HASHCHAT_DEMO=filter ./run-tui   # grim ... hashchat-tui-filter.png   (contact search / :filter term)"
echo "  HASHCHAT_DEMO=voice-sending ./run-tui  # grim ... hashchat-tui-voice-sending.png (per-contact [SENDING VOICE...] + banner)"
...
echo "  # :status + always-visible status panel"
echo "  grim -g \"\$(slurp -o)\" docs/evidence/hashchat-tui-status.png"
echo "  # Filter + QR tips in action for normal user shots"
echo "  grim -g \"\$(slurp -o)\" docs/evidence/hashchat-tui-qr-filter.png"
echo ""
