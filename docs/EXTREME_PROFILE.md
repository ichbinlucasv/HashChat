...
- **Extreme durable footprint (Rust TUI / `session_persist`)**: under Extreme, `save_session` keeps identity + onion + net prefs + disappear TTL + idle lock timeout but writes **empty** contacts / ratchets / pending / blocked / muted / verified (blob stays v7). Trade-off: no multi-session contact, deny-list, or SAS-verify continuity — re-add contacts (and re-block / re-verify if needed) after restart. `:send-unverified` is refused under Extreme. Load still accepts legacy blobs that contain contacts/deny lists into RAM for the current session; the next Extreme save strips them. Switching *to* Extreme clears the in-memory chat transcript (zeroize bodies). Idle auto-lock defaults to **1m** when previously off or longer (local UI defense; not remote wipe). Standard posture unchanged (full H3 + deny lists). **Not** Android Extreme persistence parity.
...
Last updated: overnight idle auto-lock (Extreme shortens lock-timeout to 1m when off/longer; lock timeout stays durable under Extreme).
