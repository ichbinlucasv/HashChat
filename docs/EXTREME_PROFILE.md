...
- **Extreme durable footprint (Rust TUI / `session_persist`)**: under Extreme, `save_session` keeps identity + onion + net prefs + disappear TTL but writes **empty** contacts / ratchets / pending / blocked / muted (blob stays v5). Trade-off: no multi-session contact or deny-list continuity — re-add contacts (and re-block if needed) after restart. Load still accepts legacy blobs that contain contacts/deny lists into RAM for the current session; the next Extreme save strips them. Switching *to* Extreme clears the in-memory chat transcript (zeroize bodies). Standard posture unchanged (full H3 + deny lists). **Not** Android Extreme persistence parity.
...
Last updated: overnight contact block/mute (deny lists stripped with contacts under Extreme).
