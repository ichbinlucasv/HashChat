...
- Phase 1 Android full QROT in receive processor (user-driven): Centralized generalMessageQueue + processor thread for QROT (post-decrypt detect, FFI rotate, update local contactQueues with announced recv id, system msg, Extreme safe). Integrated with bg Tor receiver, sim feeds, wipe clear. Full parity with desktop TUI queue rotation/announce. Previous FFI for proxy/queues/file chunks + I2P set + file send demo + queue rotate in sends + topbar. All with clean/push as Lucas.
  - **"continue on remaining recomendations until we finish the project" (this session + user "continue")**: Mesh full peer sync (TUI drain + processMeshIncoming with real unframe/ratchet/QROT/queue-persist + state update; Tor sync/receive real-er with UDP exercised + framed cts; mesh send fallback with rotate/announce/decoy). Email full (Core real persist/loadEncryptedEmailInbox + save using Argon2id+AES envelope + packMessageList to emails/*.enc; poll uses real receiveEmail ratchet path + I2P note; TUI :email now load/poll/send/persist with real profile pass + ratchet + outbox). Android: mesh/email actions exercise ratchet/queue FFI + feed processor; X3DH auto now inits contactQueues for full parity. Phase3 starters: Starlink detect (Tor), Relay.hs self-host (announce/queue), quantum hybrid_kex (gated), PublicChannel (Group), PAID freemium align. This continue: integrated :relay cmd in TUI (Relay module + queues), Starlink in proxy, quantum FFI, Tauri stub, cabal, pre-tag Phase3 gates, evidence scripts Phase3. Evidence tooling + docs. Ritual + Lucas push. Evidence still 0 = v0.2 hard blocker. E2EE/queues/Extreme/Tor primary solid + Phase3 entry. No stop.
...
- Android mlock: This remains a significant and honestly documented limitation. On Android, full mlockall is not reliable for normal apps. The current code only performs a best-effort single-pointer mlock that can silently fail. Real memory protection on Android comes from Keystore + app-private storage + short data lifetime + ZeroizeOnDrop + process death on wipe, not from mlock. See THREATMODEL.md and comments in src/rust/android_jni.rs.
...
- Screenshots: Detailed capture instructions + 4 descriptive slots in metainfo.xml created (docs/SCREENSHOTS.md). Actual images still needed. **User Fedora photos/evidence via scripts is now Critical hard blocker for signed v0.2** (see THREATMODEL.md updated SBOM+photos section, ROADMAP, docs/REAL_DEVICE_TESTING.md + SCREENSHOTS.md): run on real Fedora `./scripts/screenshot-prep-fedora.sh` (HASHCHAT_DEMO=main|refusal|voice|groups|actions|extreme|i2p|file ./run-tui + grim/gnome-screenshot for black+gold + posture/proxy/Extreme/queue 'i' info visible states); rsvg-convert icons; `./scripts/real-device-test.sh | tee docs/evidence/...log` (Phase1 coverage: I2P set, :file XFTP, queue rot observe in 'i' sendQ/recvQ/lastRot, Extreme refusals, long-term QR, voice/wipe/posture on Fedora/Tails + Android device); upload PNGs, edit metainfo, test Flatpak, Flathub submit for Fedora Apps marketplace. Commit evidence. No tag without.
- SBOM process (formal for signed tag; "update THREATMODEL for SBOM" completed): generate-sbom.sh supports SBOM_TAG=vX.Y (outputs sbom-vX.Y/ or sbom-pre-tag/). Pre-tag-check.sh runs formal diff vs prior sbom-<prev>/ (or committed sbom/ proxy as for 80abc0b), reviews critical crates (harder in --strict), saves diff to sbom-*/diff-vs-prior.txt + sbom/diffs/. Gates enforce artifacts + marker at exact HEAD + review. Full process + findings documented in updated THREATMODEL.md (dedicated "SBOM and Supply Chain Security (Phase 1 complete + formal for signed tag)" section) and here.
  - rust-sbom.json: 57 packages (same count); diff vs 80abc0b purely non-semantic (SPDX documentNamespace, creationInfo 'created' timestamp, packages array re-ordering; 0 semantic, no added/removed/changed package contents).
  - Critical security crates stable (no new high-risk deps like unvetted net/crypto; verbatim): ring@0.17.14 (Apache-2.0 AND ISC), zeroize@1.8.2 + zeroize_derive@1.4.3 (Apache-2.0 OR MIT), ed25519-dalek@2.2.0 (BSD-3-Clause), x25519-dalek@2.0.1 (BSD-3-Clause), argon2@0.5.3 (MIT OR Apache-2.0), hkdf@0.12.4 (MIT OR Apache-2.0), subtle@2.6.1 (BSD-3-Clause).
  ...
  - Formal requirement: semantic-clean (or explicitly reviewed+documented) diff on the critical set before any signed v0.2; run generate with tag, pre-tag --strict, record in RELEASE + THREATMODEL. See THREATMODEL.md for full updated section (findings, crates list, process steps, review mandate).
  - Recommendation per script: use before signed tags; review for new crates in ring/zeroize/dalek/argon2 etc. Supply chain stable since last SBOM run (80abc0b proxy). (See also pre-tag-check.sh, generate-sbom.sh, ROADMAP.md execution status, THREATMODEL.md)
